 ...
 *
 * NOTE: in the static build, asset paths are relative (base: "./"). We use
 * relative paths (without leading slash) so they work both at the domain
 * root and under a sub-path (e.g. GitHub Pages project pages).
 */
...
export function pickInitialImages(isPortrait: boolean) {
  pickedDay = pickRandom(getDayPool(isPortrait));
  pickedNight = pickRandom(getNightPool(isPortrait));
